...
describe("chat completions handler (unit)", () => {
	...
	test("rejects missing model", async () => {
		const { handleChatCompletions } = await import("../src/openai/chat-completions.ts");
		const client = createMockClient("Hello");
		const body = { messages: [{ role: "user", content: "Hi" }] } as unknown as ChatCompletionRequest;
		const res = await handleChatCompletions(body, client as any);
		expect(res.status).toBe(400);
		const json = (await res.json()) as { error: { message: string } };
...
describe("chat completions response format", () => {
	test("non-streaming response has correct OpenAI shape", async () => {
		...
		const body: ChatCompletionRequest = { model: "test-model", messages: [{ role: "user", content: "Hi" }] };
	...
	test("streaming response produces valid SSE", async () => {
		...
		const body: ChatCompletionRequest = { model: "test", messages: [{ role: "user", content: "Hi" }], stream: true };
		const res = await handleChatCompletions(body, mockClient as any);
		expect(res.status).toBe(200);
	...
	test("canonical tool_calls response has correct OpenAI shape", async () => {
		const { handleChatCompletions } = await import("../src/openai/chat-completions.ts");
		const toolResponse = canonical({ type: "tool_call", calls: [{ name: "exec", arguments: { command: "ls" } }] });
		const mockClient = createMockClient(toolResponse);
		const body: ChatCompletionRequest = { model: "test-model", messages: [{ role: "user", content: "List files" }], tools: [EXEC_TOOL] };
		const res = await handleChatCompletions(body, mockClient as any);
		expect(res.status).toBe(200);
		const json = (await res.json()) as ChatCompletionResponse;
	...
	test("canonical response preserves reasoning and progress with multiple tool calls", async () => {
		const { handleChatCompletions } = await import("../src/openai/chat-completions.ts");
		const mockClient = createMockClient(canonical({ type: "tool_call", content: "Vou verificar dois pontos em paralelo.", reasoning_content: "Need two independent observations.", calls: [{ name: "exec", arguments: { command: "pwd" } }, { name: "exec", arguments: { command: "ls" } }] }));
		const body: ChatCompletionRequest = { model: "test", messages: [{ role: "user", content: "Inspect" }], tools: [EXEC_TOOL], parallel_tool_calls: true };
		const res = await handleChatCompletions(body, mockClient as any);
		const json = (await res.json()) as ChatCompletionResponse;
	...
	test("canonical tool_calls stream is validated before SSE starts", async () => {
		const { handleChatCompletions } = await import("../src/openai/chat-completions.ts");
		const mockClient = createMockClient(canonical({ type: "tool_call", calls: [{ name: "exec", arguments: { command: "pwd" } }] }));
		const body: ChatCompletionRequest = { model: "test", stream: true, messages: [{ role: "user", content: "Where am I?" }], tools: [EXEC_TOOL] };
		const res = await handleChatCompletions(body, mockClient as any);
		expect(res.status).toBe(200);
		const text = await res.text();
	...
	test("canonical tool stream emits reasoning, progress, indexed calls, then tool_calls finish", async () => {
		const { handleChatCompletions } = await import("../src/openai/chat-completions.ts");
		const mockClient = createMockClient(canonical({ type: "tool_call", content: "Checking now.", reasoning_content: "Need evidence.", calls: [{ name: "exec", arguments: { command: "pwd" } }, { name: "exec", arguments: { command: "ls" } }] }));
		const body: ChatCompletionRequest = { model: "test", stream: true, messages: [{ role: "user", content: "Inspect" }], tools: [EXEC_TOOL], parallel_tool_calls: true };
		const res = await handleChatCompletions(body, mockClient as any);
		expect(res.status).toBe(200);
		const text = await res.text();
	...
	test("malformed optimized tool response fails closed with 502", async () => {
		...
		const body: ChatCompletionRequest = { model: "test", messages: [{ role: "user", content: "List files" }], tools: [EXEC_TOOL] };
		const res = await handleChatCompletions(body, mockClient as any);
	...
	test("malformed optimized tool stream returns HTTP 502 before SSE", async () => {
		...
		const body: ChatCompletionRequest = { model: "test", stream: true, messages: [{ role: "user", content: "List files" }], tools: [EXEC_TOOL] };
		const res = await handleChatCompletions(body, mockClient as any);
		expect(res.status).toBe(502);
	...
	test("multi-turn tool flow (step 4: tool result → final answer)", async () => {
		...
		const body: ChatCompletionRequest = {
			...
			messages: [
				...
				{ role: "assistant", content: null, reasoning_content: "Need directory contents.", tool_calls: [{ id: "call_abc123", type: "function", function: { name: "exec", arguments: '{"command":"ls"}' } }] },
