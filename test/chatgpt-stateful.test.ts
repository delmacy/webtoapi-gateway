...
describe("ChatGPT stateful routing", () => {
	test("preserves retry idempotence and sends only tool-result delta on continuation", async () => {
		configureAgentLayer(
			{
				mode: "optimized",
		...
		const client: WebProviderClient = {
			...
			async sendMessage(params) {
				...
				if (response === undefined) throw new Error("Unexpected extra ChatGPT provider call");
			...
			listModels: () => [{ id: "gpt-4", name: "GPT-4" }],
		};

		const session = `chatgpt-stateful-${crypto.randomUUID()}`;
