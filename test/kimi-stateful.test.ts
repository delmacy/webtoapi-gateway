...
describe("Kimi stateful routing", () => {
	test("uses stateful affinity, idempotent retry, and tool-result delta continuation", async () => {
		...
		const client: WebProviderClient = {
			...
			async sendMessage(params) {
				...
				if (response === undefined) throw new Error("Unexpected extra Kimi provider call");
			...
			listModels: () => [{ id: "moonshot-v1-32k", name: "Moonshot v1 32K" }],
		};

		const session = `kimi-stateful-${crypto.randomUUID()}`;
