...
const runId = process.env.TFG_E2E_SESSION_ID ?? `kimi-isolation-e2e-${Date.now()}`;
...
const TOOL: ToolDefinition = {
	...
	function: {
		name: "echo_fixture",
		description: "Return the supplied value to the caller for a Kimi session isolation smoke test.",
		parameters: {
			type: "object",
...
function expectStateful(response: Response, relation: string, mode: string): void {
	...
	assert(header(response, "x-webtoapi-stateful") === "true", "Expected stateful Kimi session");
	assert(
		...
		messages: [
			{
				role: "system",
				content: reset
					? `This is a divergent reset branch for isolated Kimi session ${marker}. Serialize external gateway actions only.`
					: `This is isolated Kimi session ${marker}. Serialize external gateway actions only; do not execute them locally.`,
			},
			{
				role: "user",
...
const postResetB: ChatCompletionRequest = {
	...
	messages: [
		...
		{ role: "assistant", content: "kimi-B-ok" },
		{ role: "user", content: "Reply only with kimi-B-still-isolated." },
	],
	tools: [TOOL],
	tool_choice: "auto",
};
...
assert(
	bAfterReset.json.choices[0]?.message.content?.includes("kimi-B-still-isolated"),
...
console.log(
	`PASS: Kimi A/B isolation, append-only delta continuity, and per-session epoch reset verified for ${runId}`,
);
