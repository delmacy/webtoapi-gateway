...
const model = process.env.TFG_E2E_MODEL ?? "claude-sonnet-4-6";
const apiKey = process.env.TFG_E2E_API_KEY ?? process.env.GATEWAY_API_KEY;
const sessionId = process.env.TFG_E2E_SESSION_ID ?? `claude-stateful-e2e-${Date.now()}`;
...
const TOOL: ToolDefinition = {
	...
	function: {
		name: "echo_fixture",
		description:
			"Return the supplied value to the caller. Used only by the stateful gateway smoke test.",
		parameters: {
			type: "object",
			properties: {
...
console.log(`Claude stateful E2E: ${baseUrl} model=${model} session=${sessionId}`);
...
assert(
	...
	"Expected stateful Claude session",
);
assert(
...
console.log(
	`PASS: stateful Claude continuity, retry idempotence, and delta prompt verified for ${sessionId}`,
);
