...
const sessionId = process.env.TFG_E2E_SESSION_ID ?? `chatgpt-stateful-e2e-${Date.now()}`;
...
console.log(`ChatGPT stateful E2E: ${baseUrl} model=${model} session=${sessionId}`);
...
assert(
	...
	"Expected stateful ChatGPT session",
);
assert(
...
console.log(
	`PASS: stateful ChatGPT continuity, retry idempotence, and delta prompt verified for ${sessionId}`,
);
